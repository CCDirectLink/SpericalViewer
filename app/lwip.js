...
Image.prototype.resize=function(width,height,_,cb){
    var img=new Image();
    img.bitmap=this.bitmap.resize({width:width,height:height,algorithm: "nearestNeighbor"});
    cb(null,img);
}
Image.prototype.crop=function(x,y,x2,y2,cb){
    var img=new Image();
    img.bitmap=this.bitmap.crop({top:y,left:x,width:x2-x+1,height:y2-y+1});
    cb(null,img);
}
...
Image.prototype.toBuffer=function(_,__,cb){
    ...
    this.bitmap.write(converter, {type: ImageJS.ImageType.PNG})
}
module.exports = {
    'open':function(path,_,callback){
        var img=new Image();
        img.bitmap.readFile(path).then(
            ()=>callback(null,img),
            e=>callback(e,null)
        );
    }
