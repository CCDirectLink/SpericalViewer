		...
		return function (err, files) {
			...
			gameInfo(entryPath + dataPath, containerId, function (err, data) {
				...
				if (data == null) return;
					...
					fs.readFile(entryPath + dataPath + spo.env.path.seperator + "database.json", function (err, rawData) {
						if (typeof rawData == 'undefined') return;
							var databaseJson = JSON.parse(rawData);
							console.log(databaseJson);
							spo.gameData.addData(data.shortId, "database", databaseJson);
					})

					fs.readFile(entryPath + dataPath + spo.env.path.seperator + "global-settings.json", function (err, rawData) {
						if (typeof rawData == 'undefined') return;
							var globalSettingsJson = JSON.parse(rawData);
							spo.gameData.addData(data.shortId, "globalSettings", globalSettingsJson);
					})

					fs.readFile(entryPath + dataPath + spo.env.path.seperator + "item-database.json", function (err, rawData) {
						if (typeof rawData == 'undefined') return;
							var itemsJson = JSON.parse(rawData);
							spo.gameData.addData(data.shortId, "items", itemsJson.items);
					})

					// media

					fs.readFile(entryPath + mediaPath + spo.env.path.seperator + "font" + spo.env.path.seperator + "icons-items.png", function (err, rawData) {
						if (typeof rawData == 'undefined') return;
						...
						for (var rowIndex = 0; rowIndex < iconSet.row; rowIndex++)
						{
							for (var columnIndex = 0; columnIndex < iconSet.column; columnIndex++)
							{
								...
								spo.imageData.addImage("items", iconSpecify[columnIndex] + rowIndex, rawData, "png", startX, startY, iconSet.dimension.width, iconSet.dimension.height);
							}
						}

					})

					fs.readFile(entryPath + mediaPath + spo.env.path.seperator + "gui" + spo.env.path.seperator + "menu.png", function (err, rawData) {
						if (typeof rawData == 'undefined') return;
						...
						for (var columnIndex = 0; columnIndex < iconSet.column; columnIndex++)
						{
							...
							spo.imageData.addImage("items", iconSpecify[columnIndex], rawData, "png", startX, startY, iconSet.dimension.width, iconSet.dimension.height);
						}
