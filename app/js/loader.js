...
function Loader(){
	...
	function _extractData(file, dropped, id){
		...
		_extractChangelog(folder, dropped, id, function(data){
			...
			// "/media/gui/menu.png"
			iconSpecify = ['hp', 'attack', 'defense', 'focus', 'elemHeat', 'elemCold', 'elemShock', 'elemWave'];
			iconSet = { dimension: {width: 11, height: 11, xpad: 1}, column: 5, xstart: 620, ystart: 219 };

			for (columnIndex = 0; columnIndex < iconSet.column; columnIndex++) {
				startX = (columnIndex * (iconSet.dimension.width + iconSet.dimension.xpad)) + iconSet.xstart;
