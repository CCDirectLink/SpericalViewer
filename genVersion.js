...
try {
	branch = fs.readFileSync('./.git/HEAD', 'UTF-8').substr(16).replace('\r', '').replace('\n', '');
} catch (e) {
	console.warn("Not a git repo! Assuming default branch");
	branch = "";
}

var tag; //{ name: 'v0.3.0', commit: { sha: '3c839d40a6b0268ff41e26682d078ef70e76b8af'} }
var commit; //2011-04-14T16:00:49Z
var commitHash = 0;
var commitCount = 0;
...
function getCommit(page){
	...
	var optionsCommits = {
		hostname: 'api.github.com',
		port: 443,
		path: '/repos/CCDirectLink/SpericalViewer/commits?sha=' + branch + '&page=' + page,
		method: 'GET',
		headers: {
			'User-Agent': 'SpericalViewer-genVersion'
		}
