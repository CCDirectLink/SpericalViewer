...
function Settings(){
	...
	this.langTrigger = function() {
		settings.selectedLang = $('#lang')[0]
			.options[$('#lang')[0].selectedIndex].value;
	...
	this.display = function(){
		...
		$('#storageDirName').html(
			langEntries.content['settings.storage']
		);

		$('#storageDir').html(globals.env.path.storage);


		$('#savegameDirName').html(
			langEntries.content['settings.savegame']
		);
		...
		$('#cacheDirName').html(
			langEntries.content['settings.cache']
		);
	...
	function _getTable(currentId) {
		...
		for (var i in langArray) {
			var langValue = langArray[i].langId + '_' +
				langArray[i].langIdSub;

			var langName = langArray[i].langName;

			if (langArray[i].langNameSub != null) {
				langName += ' (' +
					langArray[i].langNameSub +
					')';
			}
