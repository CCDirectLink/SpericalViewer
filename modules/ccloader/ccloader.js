...
function CCLoader(){
	const DOWNLOAD_LINK = 'https://codeload.github.com' +
		'/CCDirectLink/CCLoader/zip/master';
	...
	this.display = function(){
		if (globals.gameData.getVersions().length === 0){
			$('h2').html(
				langEntries.content['status.noGames']
			);
			return;
		}
	...
	function _getTable(){
		...
		for (var id in globals.gameData.versions){
			...
			tableString += '<tr><td>' + id +
				'</td><td>' + game.version.string +
				'</td><td id="install' + id + '">';

			if (_isInstalled(game)){
				tableString += langEntries.content[
					'ccloader.installed'
				];
			} else {
				tableString += '<button onclick=' +
			...
			tableString += '</td><td><button onclick=' +
			"\"globals.module.sharedMemory['ccloader']" +
			"['controller'].start('" + id + "')\">" +
			langEntries.content['ccloader.start'] +
			'</button></td></tr>';
...
globals.module.on('modulesLoaded', function(){
	globals.menu.add('CCLoader', function(){},
