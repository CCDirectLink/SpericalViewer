...
function Items(){
	...
	var itemData = {
		...
		versionChangeObserver: globals.gameData.registerObserver(
			function(){
	...
	this.versionTrigger = function() {
		itemData.selectedVersion = $('#versionList')[0]
			.options[$('#versionList')[0].selectedIndex].value;
		instance.updateTable();
	};

	this.display = function(){
		$('h1').html(
			langEntries.content['items.items']
		);
		...
		$('#searchLabel').html(
			langEntries.content['items.search']
		);

		if (!globals.gameData.hasGame(itemData.selectedVersion)) {
			itemData.selectedVersion = globals
	...
	this.updateTable = function(){
		// init table
		$('#itemData').html('<table>' +
			_getTable(itemData.selectedVersion,
	...
	function _getTable(version, filter) {
		...
		var tableString = '<tr><th>' +
			...
			'</th><th>' + langEntries.content['items.item'] +
			'</th><th>' + langEntries.content['items.stats'] +
			'</th></tr>';
